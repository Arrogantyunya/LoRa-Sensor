...
void SHT1x::sendCommandSHT(int _command, int _dataPin, int _clockPin)
{
  ...
  if (ack != LOW) {
    //Serial1.println("Ack Error 0");
  }
  digitalWrite(_clockPin, LOW);
  ack = digitalRead(_dataPin);
  if (ack != HIGH) {
    //Serial1.println("Ack Error 1");
  }
}
...
void SHT1x::waitForResultSHT(int _dataPin)
{
  ...
  if (ack == HIGH) {
    //Serial1.println("Ack Error 2"); // Can't do serial stuff here, need another way of reporting errors
  }
}
