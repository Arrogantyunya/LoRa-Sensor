...
Receive_Type LoRa::AT_Query_Cmd(char *cmd, unsigned char *data_buffer, unsigned char *data_len = 0)
{
    ...
    if (Receive_Len == 0)
        Serial1.println("No receive data !!!");
...
bool LoRa::LoRa_AT(unsigned char *data_buffer, bool is_query, char *cmd, char *para)
{
  ...
  switch (Return_Type){
    case OK     : Serial1.println("Set para OK");  LoRa_MHL9LF.Mode(PASS_THROUGH_MODE); return true; break;
    case ERROR  : Serial1.println("Receipt ERROR..."); LoRa_MHL9LF.Mode(PASS_THROUGH_MODE); return false; break;
    case Bytes  : 
...
void LoRa::Rewrite_ID(void)
{
    ...
    for (unsigned char i = 0; i < 8; i++){
        if (addr[i] >= 0 && addr[i] <= 9){
            addr[i] += '0';
        }else if (addr[i] >= 10 && addr[i] <= 15){
        addr[i] += '7';
        }
    }
    addr[8] = '\0';

    if(!LoRa_MHL9LF.LoRa_AT(buf, false, AT_ADDR, addr)){
        LED_SET_LORA_PARA_ERROR;
        while (1){
            iwdg_feed(); 
            Serial1.println("Set LoRa paramter ERROR, please check the LoRa module <Parameter_Init>");
            delay(3000);
        }
    }
}

void LoRa::Rewrite_GroupID(void)
{
    unsigned char buf[4] = {0};
    char addr[9];
    LoRa_MHL9LF.LoRa_AT(buf, true, AT_MADDR_, 0);

    if (buf[0] == 0x71 && buf[1] == 0x00 && buf[2] == 0x00 && buf[3] == 0x00)
        return;
    ...
    if(!LoRa_MHL9LF.LoRa_AT(buf, false, AT_MADDR, addr)){
        LED_SET_LORA_PARA_ERROR;
        while (1){
            iwdg_feed(); 
            Serial1.println("Set LoRa paramter ERROR, please check the LoRa module <Parameter_Init>");
...
{
    Serial1.println("Configurate LoRa parameters...");
    unsigned char RcvBuffer[10];
    unsigned char Status_Buffer[20];
    bool Set_Status_Flag = true;
    unsigned char i = 0;

    Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_TFREQ, "1C578DE0");
    Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_RFREQ, "1C03AE80");     
    Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_RIQ, "00"); 
    Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_NET, "00"); 
    Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_TSF, "09"); 
    Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_RSF, "09"); 
    Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_SIP, "01"); 
    Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_BW, "07"); 
    Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_POW, "14");
    Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_TIQ, "00");       

    for (unsigned char j = 0; j < i; j++){
        if (Status_Buffer[j] == false){
            Set_Status_Flag = false;
            break;
        }
    }
    if (Set_Status_Flag == false){
        LED_SET_LORA_PARA_ERROR;
        while (1){
            iwdg_feed(); 
            Serial1.println("Set LoRa paramter ERROR, please check the LoRa module <Parameter_Init>");
...
/*
 ...
 @return    : None
 */
void LoRa::Parameter_ReInit(void)
{
	Serial1.println("Configurate LoRa parameters...");
	Serial1.println("设置LORA为主机模式");
	unsigned char RcvBuffer[10];
	unsigned char Status_Buffer[20];
	bool Set_Status_Flag = true;
	unsigned char i = 0;

	Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_TFREQ, "1C03AE80");//470.0mhz
	Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_RFREQ, "1C578DE0");//475.5mhz
	Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_RIQ, "00");
	Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_NET, "00");
	Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_TSF, "09");
	Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_RSF, "09");
	Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_SIP, "00");
	Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_BW, "07");
	Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_POW, "14");
	Status_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(RcvBuffer, false, AT_TIQ, "00");
