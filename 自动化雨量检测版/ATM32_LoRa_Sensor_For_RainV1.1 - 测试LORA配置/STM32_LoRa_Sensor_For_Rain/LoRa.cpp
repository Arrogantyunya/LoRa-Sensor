...
Receive_Type LoRa::AT_Config_Cmd(char *cmd, char * para, unsigned char *data_buffer)
{
    ...
    while (LoRa_Serial.available() > 0)
	{
        if (Receive_Len >= 100) return Invalid;//无效
        addr_temp[Receive_Len++] = LoRa_Serial.read();
    }
    Receive_Len > 0 ? Is_Valid = true : Is_Valid = false;
    ...
    if (Is_Valid == true){
        if ((addr_temp[0] == '\r' && addr_temp[1] == '\n') && (addr_temp[Receive_Len - 1] == '\n' && addr_temp[Receive_Len - 2] == '\r'))
        {
           ...
           if (Detect_Error_Receipt(&addr_temp[0], data_buffer) != No_Err) return ERROR;
           if (Detect_OK_Receipt7(&addr_temp[0], data_buffer) == true) return OK;
        }else
            return Invalid;
    }else
        return Invalid;
...
/*
 ...
 @para      : data_buffer ---> Receive LoRa information.接收LoRa信息
              is_query ---> Query instruciton or set instruction.查询指令或设置指令
              *cmd ---> AT instruction.AT指令
              *para ---> AT parameter.AT参数
 @return    : true or false
 */
bool LoRa::LoRa_AT(unsigned char *data_buffer, bool is_query, char *cmd, char *para)
{
  ...
  if (is_query == true)
      Return_Type = LoRa_MHL9LF.AT_Query_Cmd(cmd, data_buffer, &Receive_length);//AT查询指令函数
  else
      Return_Type = LoRa_MHL9LF.AT_Config_Cmd(cmd, para, data_buffer);//AT设置指令函数
...
void LoRa::Rewrite_GroupID(void)
{
	...
	{
        LED_SET_LORA_PARA_ERROR;//LORA
        while (1)
		{
            iwdg_feed(); 
            Serial1.println("Set LoRa paramter ERROR, please check the LoRa module <Parameter_Init>");
			//设置LoRa参数错误，请检查LoRa模块<参数Init>
            delay(3000);
        }
    }
}
...
void LoRa::Parameter_Init(void)
{
    ...
    Bool_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(buffer, false, AT_BW, "07"); 
    Bool_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(buffer, false, AT_POW, "14");
    Bool_Buffer[i++] = LoRa_MHL9LF.LoRa_AT(buffer, false, AT_TIQ, "00");       

    for (unsigned char j = 0; j < i; j++){
        if (Bool_Buffer[j] == false){
