...
bool SN_Operations::Self_Check(unsigned char *sn_code)
{
    ...
    if (Read_SN_Code(sn_code) && Read_BKP_SN_Code(sn_code)){
        Serial1.println("Read SN code and read backup SN code success...");
        Bool_Value = true;

    }else if (Read_SN_Code(sn_code)){
        Serial1.println("Read SN code success but read backup SN code failed...");
        Save_BKP_SN_Code(sn_code) == true ? Bool_Value = true : Bool_Value = false;

    }else if (Read_BKP_SN_Code(sn_code)){
        Serial1.println("Read backup SN code success but read SN code failed...");
        Save_SN_Code(sn_code) == true ? Bool_Value = true : Bool_Value = false;

    }else{ 
        Serial1.println("All SN store ERROR!");
        Clear_SN_Save_Flag();
...
void LoRa_Config::Save_LoRa_Config_Flag(void)
{
	...
	{
		Serial1.println("LORA设置标志位保存成功!!!");
	}
	else
	{
		Serial1.println("LORA设置标志位保存失败!!!");
		Serial1.println(String("LORA_PARA_CONFIG_FLAG_ADDR = ") + AT24CXX_ReadOneByte(LORA_PARA_CONFIG_FLAG_ADDR));
	}
    EEPROM_Write_Disable();
}
...
bool Work_Parameter_Information::Save_Collect_Time(unsigned int time)
{
    ...
    if (Time_Temp == Time){
        Serial1.println("collect data already stored...");
        return true;
    }else{
        EEPROM_Write_Enable();        
