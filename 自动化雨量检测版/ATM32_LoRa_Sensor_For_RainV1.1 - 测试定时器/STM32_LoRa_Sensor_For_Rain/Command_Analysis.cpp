...
void Command_Analysis::Receive_LoRa_Cmd(void)
{
	...
	while (LoRa_Serial.available() > 0) {
		...
		delay(5);
		Serial1.print(g_Receive_cmd[g_Receive_Length - 1], HEX);
		Serial1.print(" ");
		...
		if (Verify_Device_Type_ID() == true) {
			...
			if (Verify_Area_Number() == true || area_flag == false) {
				if (Verify_Work_Group() == true || group_flag == false) {
					...
					Serial1.println("Not this device group number... <Verify_Frame_Validity>");
				}
			}
			else {
				Serial1.println("Not this device area number... <Verify_Frame_Validity>");
			}
		}
		else {
			Serial1.println("Device type ID ERROR! <Verify_Frame_Validity>");
		}
	}
	else {
		Serial1.println("CRC8 ERROR! <Verify_Frame_Validity>");
	}
	return false;
}
...
void Command_Analysis::Query_Current_Work_Para(void)
{
	...
	if (Verify_Frame_Validity(4, 23, true, false) == true) {
		...
		Serial1.println(String("服务器设置的time_temp = ") + time_temp);
		if (WorkParameter_Info.Save_Collect_Time(time_temp))
			g_Get_Para_Flag = true;
		else
			Serial1.println("Save collect time failed !!!保存收集时间失败!!");
...
void Command_Analysis::Set_Group_Number(void)
{
	...
	if (Verify_Frame_Validity(4, 10, true, false) == true) {
		if (Control_Info.Save_Group_Number(&g_Receive_cmd[8]) == true) {
			Serial1.println("Save group number success... <Set_Group_Number>");
			Serial1.println("保存组号成功…<Set_Group_Number>");
			Message_Receipt.General_Receipt(AssignGroupIdArrayOk, 1);
		}
		else {
			Serial1.println("Save group number failed !!! <Set_Group_Number>");
			Serial1.println("保存组号失败!!<Set_Group_Number>");
...
void Command_Analysis::Set_SN_Area_Channel(void)
{
	...
	if (Verify_Frame_Validity(4, 15, false, false) == true) {
		if (SN.Save_SN_Code(&g_Receive_cmd[10]) == true && SN.Save_BKP_SN_Code(&g_Receive_cmd[10]) == true) {
			Serial1.println("Set SN code success... <Set_SN_Area_Channel>");
			Serial1.println("设置序列号成功…<设置SN区域信道>");

			if (Control_Info.Save_Area_Number(g_Receive_cmd[7]) == true) {
				Serial1.println("Save area number success... <Set_SN_Area_Channel>");
				Serial1.println("保存区域号成功…<设置SN区域信道>");

				Message_Receipt.General_Receipt(SetSnAndSlaverCountOk, 1);
				SN.Set_SN_Access_Network_Flag();
			}
			else {
				Serial1.println("Save area number ERROR !!! <Set_SN_Area_Channel>");
				Serial1.println("保存区域号错误!!<设置SN区域信道>");
				Set_Motor_Status(STORE_EXCEPTION);
				Message_Receipt.General_Receipt(SetSnAndSlaverCountErr, 1);
			}
		}
		else {
			Serial1.println("Save SN code ERROR !!! <Set_SN_Area_Channel>");
			Serial1.println("保存SN码错误!!<设置SN区域信道>");
			Set_Motor_Status(STORE_EXCEPTION);
			Message_Receipt.General_Receipt(SetSnAndSlaverCountErr, 1);
		}
	}
