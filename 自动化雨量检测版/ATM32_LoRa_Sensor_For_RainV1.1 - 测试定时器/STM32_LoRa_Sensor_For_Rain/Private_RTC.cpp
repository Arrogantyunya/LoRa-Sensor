...
void date::Update_RTC(unsigned char *buffer)
{
	...
	Serial1.println("Update RTC SUCCESS... <Update_RTC>更新RTC成功……<更新RTC >");
}
...
void date::Set_Alarm(void)
{
	...
	if (WorkParameter_Info.Verify_Collect_Time_Flag())
	{
		...
		{
			Time_temp = 180;
			Serial1.println(String("设置休眠时间超过阈值，休眠时间为") + String(Time_temp) + "秒");
		}

		alarm += Time_temp;
		Serial1.println(String("服务器设置的采集时间为") + String(Time_temp) + "秒");
	}
	else
	{
		alarm += 180;
		Serial1.println("设置休眠时间超过阈值，休眠时间为180秒");
	}
