...
void Receipt::Report_General_Parameter(void)
{
  ...
  Serial1.println("Report general parameter...");
...
void Receipt::Request_Set_Group_Number(void)
{
  ...
  Serial1.println("Requeset SN code to access network...");
...
{  
  ...
  Serial1.println("LoRa parameter receipt...");
...
void Receipt::General_Receipt(unsigned char status, unsigned char send_times)
{
  ...
  Serial1.println("Send General Receipt...");
  Print_Debug(&Receipt_Frame[0], Receipt_Length);
...
void Receipt::Send_Sensor_Data(void)
{
  ...
  Serial1.println("LoRa sensor data receipt...");
...
/*
 @brief   : 串口打印16进制回执信息
            Serial1 port prints hex receipt information.
