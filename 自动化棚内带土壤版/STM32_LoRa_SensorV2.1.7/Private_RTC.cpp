...
void date::Update_RTC(unsigned char *buffer)
{
	...
	Serial.println("Update RTC SUCCESS... <Update_RTC>");//更新RTC成功……<更新RTC >
}
...
void RTC_Interrupt(void)
{
	...
	rtc_detach_interrupt(RTC_ALARM_SPECIFIC_INTERRUPT);//关闭RTC中断
	nvic_sys_reset();//复位
}
...
void date::Set_Alarm(void)
{
	...
	if (WorkParameter_Info.Verify_Collect_Time_Flag())//WorkParameter信息.验证收集时间标志
	{
		unsigned int Time_temp = WorkParameter_Info.Read_Collect_Time();//阅读收集时间
