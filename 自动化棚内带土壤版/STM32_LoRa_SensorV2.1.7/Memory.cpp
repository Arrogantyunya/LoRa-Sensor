...
bool SN_Operations::Self_Check(unsigned char *sn_code)
{
	...
	{
		Serial.println("Read SN code and read backup SN code success...");//读取SN并读取备份SN成功…
		Bool_Value = true;

	}
	else if (Read_SN_Code(sn_code)) 
	{
		Serial.println("Read SN code success but read backup SN code failed...");//读取SN代码成功，但读取备份SN代码失败…
		Save_BKP_SN_Code(sn_code) == true ? Bool_Value = true : Bool_Value = false;

	}
	else if (Read_BKP_SN_Code(sn_code)) 
	{
		Serial.println("Read backup SN code success but read SN code failed...");//读取备份SN代码成功，但读取SN代码失败…
		Save_SN_Code(sn_code) == true ? Bool_Value = true : Bool_Value = false;

	}
	else 
	{
		Serial.println("All SN store ERROR!");//所有SN存储错误!
		Clear_SN_Save_Flag();
...
bool Work_Parameter_Information::Save_Collect_Time(unsigned int time)
{
	...
	if (Time > 43200 || Time < 15)  //max: 12 hours; min: 15seconds.
		return false;
