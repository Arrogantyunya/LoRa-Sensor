...
unsigned char g_SN_Code[9] = {0x00, 0x01, 0x20, 0x18, 0x11, 0x26, 0x15, 0x00, 0x05};
...
void setup() 
{
  ...
  unsigned char End_Frame[9] = {0xAA, 0x03, 0x18, 0x0D, 0x0A, 0x0D, 0x0A, 0x0D, 0x0A};
...
void Set_Alarm(void)
{
  ...
  unsigned long int alarm2 = InRtc.getTime(); //Get current time.
  ...
  //InRtc.createAlarm(RTC_Interrupt, alarm2);

  InRtc.setAlarmTime(alarm2);
  delay(100);
  bkp_disable_writes();
}
...
void Read_Salt_and_Cond(unsigned int *salt, unsigned int *cond, unsigned char addr)
{
  ...
  if (Receive_CRC16 == Verify_CRC16)
  {
    salt_temp = Receive_Data[3] << 8 | Receive_Data[4];
    cond_temp = Receive_Data[5] << 8 | Receive_Data[6];
  }
...
void LoRa_Send_Sensor_Data(void)
{
  ...
  Serial.println(Sensor_Data.g_Solid_Temp);
