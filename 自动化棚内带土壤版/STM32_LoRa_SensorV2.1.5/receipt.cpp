...
void Receipt::Request_Set_Group_Number(void)
{
  ...
  Request_Frame[Frame_Length++] = 0x00; //Master device
...
{  
  ...
  //Voltage value.
  Receipt_Frame[Receipt_Length++] = 0x00;
  Receipt_Frame[Receipt_Length++] = 0x18; //24V
  //SNR and RSSI 
...
void Receipt::General_Receipt(unsigned char status, unsigned char send_times)
{
  ...
  //Area Number.
  Receipt_Frame[Receipt_Length++] = Control_Info.Read_Area_Number();
  Receipt_Frame[Receipt_Length++] = 0x00; //Master device
...
void Receipt::Send_Sensor_Data(void)
{
  ...
  Sensor_Buffer[Receipt_Length++] = Type_Conv.Dec_To_Hex(highByte(BatVol));
  Sensor_Buffer[Receipt_Length++] = Type_Conv.Dec_To_Hex(lowByte(BatVol));
